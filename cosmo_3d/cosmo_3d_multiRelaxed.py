...
def run_example(dt,X,Xi,kppa,lap_fac,omega_m,t_ini,T,L,imx,u_ini,energy_type=None,exact_soln_np=None,log_errs=False,num_plots=100,p=3.0,
    ...
    def sum_contributions(dt,t,psi,s,im_B,ex_B,emb_B,im_K,ex_K):
        ...
        return jnp.stack([term,term_emb],axis=0)
   ...
   #jac_psi_dt = jax.jit(jax.jacfwd(time_stepper,0))
    ...
    def tobe_minimized(gamma,args):
        ...
        t = args[6]
        dt = args[7]
        tn = t + dt*(1.0+jnp.sum(gamma))
 

        psi_new = psi + term1*(1.0+gamma[0]) + term2*gamma[1]
        mass_new = jnp.mean(jnp.abs(psi_new)**2)#mass(psi_new)
        energy_new = calc_energy(tn,psi_new)
        



        return jnp.array([mass_new - mass_old,energy_new-energy_old-Q_int*(1.0+jnp.sum(gamma))]) #+ eqn
    ...
    gamma_0 = jnp.array([0.0,0.0],dtype=jnp.float64)
    Q_int_l = 0.0
    fail_cntr = 0
    fail_cntr_step=0
    while (t<tmax):

        #print("Time step n=",n," at time t=",t," with dt=",dt,flush=True)
        ...
        if opt_algo=="optx_Newton" or opt_algo=="optx_IndirectLevenbergMarquardt":
            sol = optx.root_find(tobe_minimized, solver, gamma_0, args=(term1,term2,psi,mass_now,energy_now,Q_int,t,dt), max_steps=4,throw=throw)
            gamma = sol.value
        elif opt_algo=="jaxopt_scipy":
            
            sol = solver.run(gamma_0, args=(term1,term2,psi,mass_now,energy_now,Q_int,t,dt))
            gamma = sol.params
        elif opt_algo=="scipy_root":
            
            #sol = root(tobe_minimized,gamma_0,args=(term1,term2,psi,mass_now,energy_now,Q_int,t), method='hybr',tol=1e-14)
            sol = root(tobe_minimized,gamma_0,args=((term1,term2,psi,mass_now,energy_now,Q_int,t,dt),), method='hybr',tol=1e-14)
            
            gamma = sol.x

        elif opt_algo=="jaxopt_broyden":
            brd = Broyden(fun =tobe_minimized,tol=1e-12,stop_if_linesearch_fails = True)
            sol = brd.run(gamma_0, args=(term1,term2,psi,mass_now,energy_now,Q_int,t,dt))
            gamma = sol.params
        else:
        ...
        fval = tobe_minimized(gamma,(term1,term2,psi,mass_now,energy_now,Q_int,t,dt))
        #print("gama=",gamma," func val:",fval)
        #print(" gamma=",gamma,1.0+jnp.sum(gamma),flush=True)
        #print("sol",sol.value)#,info)
        if jnp.max(jnp.abs(fval))>1e-12 or (1.0+jnp.sum(gamma))<0.0 or jnp.isnan(jnp.sum(gamma)):
            print("Warning: Root finding in relaxation step did not converge, func value:",fval,(1.0+jnp.sum(gamma)),flush=True)
            fail_cntr = fail_cntr+1
            if jnp.isclose(dt,dt_orig):
                 fail_cntr_step = fail_cntr_step+1
            gamma = gamma_0#jnp.array([0.0,0.0],dtype = jnp.float64)
            opt_succ=False
            dt = dt*0.5
            print("Reducing dt to ",dt," and retrying",flush=True)
            if dt<1e-10:
                print("dt too small, exiting",flush=True)
        ...
        #print("func value:",tobe_minimized(sol.value,(term1,term2,psi,mass_ini,calc_energy(psi))))
            ...
            t = t+(1.0+jnp.sum(gamma))*dt
            Q_int_l += Q_int*(1.0+jnp.sum(gamma))
            dt = dt_orig
        ...
        if n%int(nplt)==0:
            print("t=",t,"1+ gamma sum=",1.0+jnp.sum(gamma),gamma,flush=True)
            mass_relative_err = (mass_new - mass_ini)/mass_ini
            energy_err = (energy_new - energy_ini-Q_int_l)#/energy_ini
            energy_err_loc = (energy_new - energy_now-Q_int*(1.0+jnp.sum(gamma)))#/energy_ini
            mass_err_list.append(mass_relative_err)
            energy_err_list.append(energy_err)
            energy_err_loc_list.append(energy_err_loc)
            t_list.append(t)
            print(t,dt,"Mass new:",mass_new,"Relative Mass diff:",mass_relative_err,"Energy diff:",energy_err,flush=True)
            if data_dir is not None:
        ...
        n=n+1
    print("t=",t,"1+ gamma sum=",1.0+jnp.sum(gamma),gamma)
    mass_relative_err = (mass_new - mass_ini)/mass_ini
    energy_err = (energy_new - energy_ini-Q_int_l)#/energy_ini
    energy_err_loc = (energy_new - energy_now-Q_int*(1.0+jnp.sum(gamma)))#/energy_ini
    mass_err_list.append(mass_relative_err)
...
if __name__=="__main__":
    ...
    if energy_type=="E1":
        save_dir = "./_dataij_hunit_TSC_cosmo_I_multiRlx_E1_v"
    elif energy_type=="E2":
        save_dir = "./_dataij_hunit_TSC_cosmo_I_multiRlx_E2_v"
    else:
