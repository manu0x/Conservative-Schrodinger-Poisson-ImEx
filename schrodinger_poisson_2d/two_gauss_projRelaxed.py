   ...
   #jac_psi_dt = jax.jit(jax.jacfwd(time_stepper,0))
    ...
    while (t<tmax):
        ...
        if opt_algo=="optx_Newton" or opt_algo=="optx_IndirectLevenbergMarquardt":
            sol = optx.root_find(tobe_minimized, solver, gamma, args=(term_proj,psi,mass_now,energy_now,t,dt), max_steps=10,throw=throw)
            gamma = sol.value
        elif opt_algo=="jaxopt_scipy":
            
            sol = solver.run(gamma, args=(term_proj,psi,mass_now,energy_now,t,dt))
            gamma = sol.params

        elif opt_algo=="scipy_root_scalar":
            
            #sol = root(tobe_minimized,gamma_0,args=(term1,term2,psi,mass_now,energy_now,t), method='hybr',tol=1e-14)
            sol = root_scalar(tobe_minimized_srscalar,args=((term_proj,psi,mass_now,energy_now,t,dt),),x0 = gamma_0, method='secant',xtol=1e-14)
            
            gamma = sol.root
        elif opt_algo =="scipy_fsolve":
            
            sol = fsolve(tobe_minimized_srscalar,gamma_0,args=((term_proj,psi,mass_now,energy_now,t,dt),),xtol=1e-14)
            gamma = sol[0]
        elif opt_algo=="jaxopt_broyden":
            
            sol = brd.run(gamma, args=(term_proj,psi,mass_now,energy_now,t,dt))
            gamma = sol.params
        else:
        ...
        fval = tobe_minimized(gamma,(term_proj,psi,mass_now,energy_now,t,dt))
...
if __name__=="__main__":
    ...
    if energy_type=="E1":
        save_dir = "./_data_cntr_twoGauss_projRlx_E1/"
    elif energy_type=="E2":
        save_dir = "./_data_cntr_twoGauss_projRlx_E2/"
    else:
