...
def run_example(dt,X,Xi,kppa,t_ini,T,L,imx,u_ini,energy_type=None,exact_soln_np=None,log_errs=False,lap_fac=1.0,num_plots=100,p=3.0,
   ...
   #jac_psi_dt = jax.jit(jax.jacfwd(time_stepper,0))
    ...
    def tobe_minimized(gamma,args):
        ...
        mass_new = np.mean(np.abs(psi_new)**2)#mass(psi_new)
        energy_new = calc_energy(tn,psi_new)
        



        return np.array([mass_new - mass_old,energy_new-energy_old-Q_int*(1.0+np.sum(gamma))]) #+ eqn
    ...
    while (t<tmax):
        ...
        if np.max(np.abs(fval))>1e-12 or (1.0+np.sum(gamma))<0.0 or np.isnan(np.sum(gamma)):
        ...
        #print("func value:",tobe_minimized(sol.value,(term1,term2,psi,mass_ini,calc_energy(psi))))
            ...
            Q_int_l += Q_int*(1.0+np.sum(gamma))
            dt = dt_orig
        ...
        if n%int(nplt)==0:
            ...
            energy_err_loc = (energy_new - energy_now-Q_int*(1.0+np.sum(gamma)))#/energy_ini
...
if __name__=="__main__":
    ...
    if energy_type=="E1":
        save_dir = "./_data_sine_wave_multiRlx_E1/"
    elif energy_type=="E2":
        save_dir = "./_data_sine_wave_multiRlx_E2/"
    else:
